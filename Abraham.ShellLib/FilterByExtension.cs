...
namespace Abraham.ShellLib
{
	...
	public class FilterByExtension : IFolderFilter
	{
			...
			IntPtr pidlItem)		// The item's PIDL.
		{
			...
			Abraham.ShellLib.IShellFolder isf = (Abraham.ShellLib.IShellFolder)psf;

			Abraham.ShellLib.ShellApi.STRRET ptrDisplayName;
			isf.GetDisplayNameOf(pidlItem,(uint)Abraham.ShellLib.ShellApi.SHGNO.SHGDN_NORMAL | (uint)Abraham.ShellLib.ShellApi.SHGNO.SHGDN_FORPARSING,out ptrDisplayName);
			
			String sDisplay;
			Abraham.ShellLib.ShellApi.StrRetToBSTR(ref ptrDisplayName,(IntPtr)0,out sDisplay);
			
			// check if item is file or folder
			IntPtr[] aPidl = new IntPtr[1];
			aPidl[0] = pidlItem;
			uint Attrib;
			Attrib = (uint)Abraham.ShellLib.ShellApi.SFGAO.SFGAO_FOLDER;

			int temp;
			temp = isf.GetAttributesOf(1,aPidl,ref Attrib);

			// if item is a folder, accept
			if ((Attrib & (uint)Abraham.ShellLib.ShellApi.SFGAO.SFGAO_FOLDER) == (uint)Abraham.ShellLib.ShellApi.SFGAO.SFGAO_FOLDER)
				return 0;
